# 🧠 Learning Notes – CodA-Nova Navbar-03

## Example: Navigation Menu justify-content Issue

While coding the third template, the navigation menu container, I initially used `justify-content: space-around'** instead of **'space-between`.
nav {
    display: flex;
...
This issue helped me better understand the behavioral difference between 
`space-around` and `space-between`:

- `space-around` adds equal space around each item
- `space-between` distributes space only between items

Choosing the correct alignment strategy significantly impacts visual balance.
